...
trial_end=20
large_time=100 # a second far beyond trial_end
...
MyHidden <- function(glist,plist,sqc,k_e,r_e,pe_time,t1,t0,k_pe,r_pe,k_ge,r_ge){
  ...
  #for E->->E
  #no hidden baserate
  prob_sub_e=dgamma(t1-t0, shape=k_e, rate=r_e) %>% c(prob_sub_e)
  #hidden baserate
  ...
  if (nrow(p_between)>0){
    ...
    prob_sub_e=prob_segment %>% c(prob_sub_e)
  ...
  return(max(prob_sub_e))
}

MyEndHidden <- function(glist,plist,sqc,k_e,r_e,e_index,pe_time,t1,t0,k_pe,r_pe,k_ge,r_ge){
  prob_sub=c()
  j=max(e_index)
  e_time=sqc$time[j]
  #no hidden baserate
  prob_sub=(1-pgamma(trial_end-e_time, shape=k_e, rate=r_e)) %>% c(prob_sub)
  #hidden baserate
  #using approximation approach
  ...
  if (nrow(p_between)>0){
    ...
    prob_sub=prob_segment %>% c(prob_sub)
    ...
    for (m in glist){
      ...
      for (j in cdd_index){
        ...
        if (sum(cdd_path$cause==m & cdd_path$cause_idx==sqc$obj_idx[j])==0){
          prob_sub_g=c()
          prob_sub_g=(1-pgamma(trial_end-cdd_time, shape=k_ge, rate=r_ge)) %>% c(prob_sub_g)
          for (q in p_index){
            ...
            prob_sub_g=max(summary(ecdf(prevent_prob))) %>% c(prob_sub_g)
          }
          prob_sub=prob_sub*max(prob_sub_g)
        }
      }
      prob=prob*prob_sub
    }
    #replicating on E baserate
    prob_sub=MyEndHidden(glist,plist,sqc,k_e,r_e,e_index,pe_time,t1,t0,k_pe,r_pe,k_ge,r_ge)
    prob=prob*prob_sub
...
MyOnlineInfer_one <- function(sqc_raw_long,k_e,r_e,k_pe,r_pe,k_ge,r_ge){
