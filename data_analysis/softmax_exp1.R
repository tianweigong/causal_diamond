...
for (k in 1:nrow(mdall)){
  sub=df.final %>% subset(trial_id==mdall$seed[k] &
                          choice==mdall$choice[k])
...
save(CV.list,BIC.list,file = "modelFit_exp1.Rda")
...
MyIndFit<-function(subk){
  ...
  for (k in 1:nrow(mdsub)){
    sub=df.final %>% subset(subject==subk &seed==mdall$seed[k]&
                              choice==mdall$choice[k] )
