...
myMatPlot<-function(d){
  ...
  geom_tile()+
  geom_text(aes(label = mylabel,color=label_col),size=6)+
...
# ggsave(file="exp_mat_ppl.pdf",width = 7,height = 4)
...
# ggsave(file="exp_mat_nor.pdf",width = 7,height = 4)

md.sim[["feai"]] %>% mutate(pro=A_pro,state=A_state)%>%
  rbind(mutate(md.sim[["feai"]],pro=B_pro,state=B_state))%>%  myMatPlot()+
  ylab("Model")

# ggsave(file="exp_mat_feai.pdf",width = 7,height = 4)

md.sim[["feaw"]] %>% mutate(pro=A_pro,state=A_state)%>%
  rbind(mutate(md.sim[["feaw"]],pro=B_pro,state=B_state))%>%  myMatPlot()+
  ylab("Model")

# ggsave(file="exp_mat_feaw.pdf",width = 7,height = 4)
```
...
df.comb %>% mutate(paired=B_state,state=A_state,gd=A_pro)%>%
  rbind(mutate(df.comb,paired=A_state,state=B_state,gd=B_pro))%>%
  myPairFrePlotGd()+
  ylab("Proportion")+
  theme(legend.position ="right")
...
```{r}
x=df.comb %>% mutate(pro=A_pro,state=A_state,paired=B_pro)%>%
  rbind(df.comb %>% mutate(pro=B_pro,state=B_state,paired=B_pro))%>%
  filter(pro=="N"& state!="N" & mycondition=="irregular")
chisq.test(table(x$paired,x$state))
```
...
tmp%>%subset(mycondition=="irregular" & gd=="P")
aov(acc~ord,tmp%>%subset(mycondition=="regular" & gd=="P")) %>% summary()
