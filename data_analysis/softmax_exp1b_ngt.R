...
load("exp3.Rda")
df.final=df.final %>% mutate(choice=paste(A_state,B_state,sep=""))%>%
  subset(seed>9)
load('mdall_exp3.Rda')
mdall=mdall%>%subset(seed>9)
...
for (k in c(2,2.5,3,3.5,4,4.5,5)){
...
save(CV.list,BIC.list,file = "modelFit_exp3_ngt.Rda")
...
loglen=18
fldnew="softmax_exp1b_ngt"
wholelist=as.list(as.character(unique(df.dmg$subject)))
mclapply(wholelist,MyIndFit,mc.cores=7)
