...
MyNorMdRun<-function(seed_fld){
  ...
  for (k in 1:length(sti_file)){
    ...
    if (seed_fld>9){eval(parse(text=paste("sqc.",substr(sti_file[k],1,4),"=sqc",sep="")))}
  }
  md=data.frame()
...
filepath="sti_exp1b"
new_filepath="model_exp1b"

##calculate effect number information
seed.eff=data.frame()
for (seed_fld in 1:12){
  sti_file=list.files(path = paste(filepath,"/",'seed',seed_fld,sep = ""),pattern = "\\.Rda$")
  for (k in 1:length(sti_file)){
    load(paste(filepath,"/",'seed',seed_fld,"/",sti_file[k],sep = ""))
    if (seed_fld>9){eval(parse(text=paste("sqc.",substr(sti_file[k],1,4),"=sqc",sep="")))}
  ...
  df$uni_seed=paste("exp1b",seed_fld,sep="_")
  for (k in 1:length(sti_list)){
    sqc_raw_long=get(sti_list[k])
...
save(seed.eff,file="eff_exp1b.Rda")
##



#normative
wholelist=as.list(c(1:12))
...
save(mdall,file="mdall_exp1b.Rda")
