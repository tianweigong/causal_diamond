...
pic.model=pic.cb %>%  gather(model, ratio, a_normative:b_feature)
...
md9.nor$sfm=NA
for (i in 1:18){
  choice_sub=md9.nor %>% subset(trial_id==i)
  for (m in 1:9){
    sfm=exp(choice_sub$ratio[m]*a)/sum(exp(choice_sub$ratio*a))
    md9.nor$sfm[which(md9.nor$trial==i)[m]]=sfm
  }
}
colnames(md9.nor)[ncol(md9.nor)]="a_normative"
md9.nor$ratio=NULL
...
md9.fea=merge(md.delay,md.num,by=c("trial_id","A_pro","B_pro","A_state","B_state"))
md9.fea$sfm=NA
for (i in 1:18){
  choice_sub=md9.fea %>% subset(trial_id==i)
  for (m in 1:9){
    sfm=exp(choice_sub$ratio.x[m]*a1+choice_sub$ratio.y[m]*a2)/sum(exp(choice_sub$ratio.x*a1+choice_sub$ratio.y*a2))
    md9.fea$sfm[which(md9.fea$trial==i)[m]]=sfm
  }
}
colnames(md9.fea)[ncol(md9.fea)]="b_feature"
md9.fea$ratio.x=NULL
md9.fea$ratio.y=NULL

pic.cb=merge(md9.nor,md9.fea,by=c("trial_id","A_pro","B_pro","A_state","B_state"))
...
md9.sub=md9.sub%>%
  mutate(trial=pic.label[trial_id],
         state=paste(md9.sub$A_state,md9.sub$B_state,sep=""))
pic.model=pic.model%>%
  mutate(trial=pic.label[trial_id],
         state=paste(pic.model$A_state,pic.model$B_state,sep=""))

ggplot()+
  geom_bar(md9.sub,mapping=aes(x=state, y=hm),stat='identity',position="dodge",color="black")+
  geom_point(pic.model,mapping =aes(x=state, y=ratio,shape=model), stat="identity",
             position=position_dodge(.9),size=1.8)+
  facet_wrap(~trial,nrow=3)+
  theme_bw()+
  theme(text = element_text(size=16),
...
#Feature-based model picture
load("../model/feature_based/df.expect.Rda")

df.expect %>% subset(rel=="r") %>% ggplot(aes(d_average))+
  # geom_histogram(aes(y=..density..),      # Histogram with density instead of count on y-axis
  #                colour="black") +
  geom_density(aes(group = A_pro,fill=A_pro))+ 
  theme_bw()+
  ...
  xlab("Delay")+
  scale_fill_manual(values=alpha(c("#D5E7E1","#EDB1A4","#6282b1"),0.7),
                    labels =  c(" Generative"," Non-causal"," Preventative"))+
  scale_x_continuous(limits = c(0,17))

x=df.expect %>% subset(rel=="r" & A_pro=="P") 
mean(x$A_delay1)
sd(x$A_delay1)

#df.expect$c_average=(df.expect$A_num1+df.expect$A_num2+df.expect$A_num3)/3

df.e%>% subset(rel=="r" & model %in% c("A_num1","A_num2","A_num3")) %>% ggplot(aes(ratio, fill = A_pro))+
  geom_bar(color="black")+
  scale_x_continuous(limits = c(-1,3.5),breaks = c(0,1,2,3))+
  theme_bw()+
  theme(text = element_text(size=22),
        legend.title = element_blank(),
        # panel.grid.major = element_blank(),
  ...
  ylab("Simulation (x10,000)")+
  xlab("Count")+
  scale_fill_manual(values=c("#D5E7E1","#EDB1A4","#6282b1"),
                     labels =  c("Generative ","Non-causal ","Preventative "))+
  scale_y_continuous(breaks = c(0,25000*3,50000*3,75000*3,100000*3),labels = c(0,2.5,5,7.5,10))

# multiplot(p1,p2,cols=2)

ggsave(paste(3,".pdf",sep = ""), width = 6, height = 5.5)
