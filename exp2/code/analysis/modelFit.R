...
#' # load human data
load("df.final.Rda")
#the first half trials
# df.final=df.final %>% subset(trial_order<10)
#the second half trials
# df.final=df.final %>% subset(trial_order>9)
#human acc
# df.final$acc_device[df.final$mycondition=="regular"] %>%mean()
# df.final$acc_device[df.final$mycondition=="irregular"] %>%mean()
...
#+ calculate model acc -------------------
#' #  calculate model acc
md.nor %>% subset(A_pro==A_state & B_pro==B_state &ru_pro=="r",select="ratio")%>% colMeans()
md.one %>% subset(A_pro==A_state & B_pro==B_state &ru_pro=="r",select="ratio")%>% colMeans()
md.fea.w %>% subset(A_pro==A_state & B_pro==B_state &ru_pro=="r",select=c("ratio1","ratio2"))%>% colMeans()
md.fea.i %>% subset(A_pro==A_state & B_pro==B_state &ru_pro=="r",select=c("ratio1","ratio2"))%>% colMeans()

md.nor %>% subset(A_pro==A_state & B_pro==B_state &ru_pro=="u",select="ratio")%>% colMeans()
md.one %>% subset(A_pro==A_state & B_pro==B_state &ru_pro=="u",select="ratio")%>% colMeans()
md.fea.w %>% subset(A_pro==A_state & B_pro==B_state &ru_pro=="u",select=c("ratio1","ratio2"))%>% colMeans()
md.fea.i %>% subset(A_pro==A_state & B_pro==B_state &ru_pro=="u",select=c("ratio1","ratio2"))%>% colMeans()
...
myCVcal1 <-function(md,mdname,currentset){
  ...
  rel=c(mdname,
        as.numeric(x$details$par),
        NA,
        log(nrow(vals))+2*as.numeric(x$details$value),
        currentset,
...
myCVcal2 <-function(md,mdname,currentset){
  ...
  rel=c(mdname,
        as.numeric(x$details$par[1]),
        as.numeric(x$details$par[2]),
        log(nrow(vals))*2+2*as.numeric(x$details$value),
        currentset,
