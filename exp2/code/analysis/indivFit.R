...
MyIndivFit<-function(df.sig,subid){
  ...
  df.indiv.sub[nrow(df.indiv.sub)+1,]=c(subid,"normative",as.numeric(x$par),NA,log(nrow(vals)/9)+2*as.numeric(x$value))
  #one-actual
  vals=merge(md.one,df.sig,by=c("fit_id","A_pro","B_pro","A_state","B_state"))
  x=optim(0.5,ind_mod1,vals=vals,method = "Brent",lower = -1000,upper=1000)
  df.indiv.sub[nrow(df.indiv.sub)+1,]=c(subid,"one_actual",as.numeric(x$par),NA,log(nrow(vals)/9)+2*as.numeric(x$value))
  ...
  df.indiv.sub[nrow(df.indiv.sub)+1,]=c(subid,"feature_window",as.numeric(x$par[1]),as.numeric(x$par[2]),log(nrow(vals)/9)*2+2*as.numeric(x$value))
  #delay(window-based)
  vals=merge(md.fea.w.d,df.sig,by=c("fit_id","A_pro","B_pro","A_state","B_state"))
  x=optim(0.5,ind_mod1,vals=vals,method = "Brent",lower = -1000,upper=1000)
  df.indiv.sub[nrow(df.indiv.sub)+1,]=c(subid,"delay_window",as.numeric(x$par),NA,log(nrow(vals)/9)+2*as.numeric(x$value))
  #count(window-based)
  vals=merge(md.fea.w.c,df.sig,by=c("fit_id","A_pro","B_pro","A_state","B_state"))
  x=optim(0.5,ind_mod1,vals=vals,method = "Brent",lower = -1000,upper=1000)
  df.indiv.sub[nrow(df.indiv.sub)+1,]=c(subid,"num_window",as.numeric(x$par),NA,log(nrow(vals)/9)+2*as.numeric(x$value))
  
  #feature-based (intervention-based)
  vals=merge(md.fea.i,df.sig,by=c("fit_id","A_pro","B_pro","A_state","B_state"))
  x=optim(c(0.5,0.5),ind_mod2,vals=vals)
  df.indiv.sub[nrow(df.indiv.sub)+1,]=c(subid,"feature_intervention",as.numeric(x$par[1]),as.numeric(x$par[2]),log(nrow(vals)/9)*2+2*as.numeric(x$value))
  #delay(intervention-based)
  vals=merge(md.fea.i.d,df.sig,by=c("fit_id","A_pro","B_pro","A_state","B_state"))
  x=optim(0.5,ind_mod1,vals=vals,method = "Brent",lower = -1000,upper=1000)
  df.indiv.sub[nrow(df.indiv.sub)+1,]=c(subid,"delay_intervention",as.numeric(x$par),NA,log(nrow(vals)/9)+2*as.numeric(x$value))
  #count(intervention-based)
  vals=merge(md.fea.i.c,df.sig,by=c("fit_id","A_pro","B_pro","A_state","B_state"))
  x=optim(0.5,ind_mod1,vals=vals,method = "Brent",lower = -1000,upper=1000)
  df.indiv.sub[nrow(df.indiv.sub)+1,]=c(subid,"num_intervention",as.numeric(x$par),NA,log(nrow(vals)/9)+2*as.numeric(x$value))
